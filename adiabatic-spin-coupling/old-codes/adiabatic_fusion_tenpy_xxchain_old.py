...
class AdiabaticHamiltonian(XXZChain):
    def init_terms(self, model_params):
        ...
        for non_coupling_index in get_non_interaction_term_indicies([2,2]):
            c_arr[non_coupling_index] = model_params.get('time', 0)/1

        model_params['Jxx'] = c_arr * -4
        print("!!!!")
...
def complete_adiabatic_evolution_run(initial_model, final_model, dmrg_params, tebd_params, total_time, verbose=True):
    ...
    return time_evolution_engine.psi
...
if __name__ == "__main__":
    ...
    #SHAPE_F = [16]
    L = sum(SHAPE)
    total_runtimes = np.linspace(0,5,10)
    EPSILON_RODEO = 0.1
   
    from tenpy.models.xxz_chain import XXZChain2

    mu = 0
    new_hamiltonian = XXZChain2({"Jxx":J, "Jz":0, "hz":mu, "L":L})
    Z_operator = XXZChain2({'Jxx':0,"Jz":0,'hz':1,"L":L}).calc_H_MPO()
